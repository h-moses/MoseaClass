...
public class ViewUtils {
    ...
    public static void initGridLayout(Context context, ViewGroup parent, GridLayout gridLayout, int rowCount, int colCount, List<CourseIntroduction> list) {
        gridLayout.setRowCount(rowCount);
        gridLayout.setColumnCount(colCount);
        for (int i = 0; i < list.size(); i++) {
            CourseIntroduction introduction = list.get(i);
            Log.e(TAG, "initGridLayout: " + introduction.toString());
            CourseCardView courseCardView = new CourseCardView(context, parent);
            ...
            gridLayout.addView(courseCardView);
        }
    }
}
